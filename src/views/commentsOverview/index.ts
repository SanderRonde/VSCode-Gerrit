...
async function updatePanel(
  ...
  if (currentRevision) {
    await prefetchFileDiffContent(
      change, commentsMap, currentRevision
    );
  }
 ...
 * the cache.
 */
async function warmCacheShortId(
  shortId: string
): Promise<void> {
  try {
    const sub =
      await GerritChange.getAllComments(shortId);
    await sub.getValue();
  } catch {
    // ignore
  }
 ...
 */
async function warmChangeCacheShortId(
  shortId: string
): Promise<void> {
  try {
    await GerritChange.getChangeOnce(
...
async function prefetchFileDiffContent(
  ...
  await Promise.all(filePaths.map(
    async (filePath) => {
      ...
      try {
        await FileTreeView.getFileDiffContent(
          file, null
        );
      } catch {
...
> {
  ...
  const rootOf = (
    c: GerritComment | GerritDraftComment
  ): string => {
    let cur = c;
    while (
      cur.inReplyTo && byId.has(cur.inReplyTo)
    ) {
    ...
    for (const threadComments
      ...
      for (const c of threadComments) {
        ...
        if (c.isDraft) {
          ...
          threadUnresolved && isOlderPatchset
        ) {
          if (
            !olderPatchsetByFile.has(filePath)
          ) {
            olderPatchsetByFile.set(
              filePath, []
            );
          }
          olderPatchsetByFile
...
async function navigateToComment(
  ...
  try {
    ...
    if (!currentRevision) {
      log(
        'navigateToComment: revision not found'
      );
      return;
    }
    ...
    if (!diffCmd?.arguments) {
      log(
        'navigateToComment: diff command'
        + ' failed for ' + filePath
      );
      return;
    }
    ...
    // appear quickly.
    const findMgr = ():
      import('../../providers/commentProvider')
      .DocumentCommentManager | null =>
      CommentManager.getFileManagerForUri(
        rightUri
      )
      ?? CommentManager.getFileManagerForUri(
        leftUri
